...
example_pg_id <- example_data$countdata %>%
  dplyr::select("id")
...
```{r}
gimap_dataset <- run_qc(gimap_dataset)
```
...
```{r}
gimap_dataset$transformed_data$qc_filter
```

Future work will also produce an HTML report for all of the plots and tables made while running the QC process.
