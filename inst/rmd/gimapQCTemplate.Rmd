...
```{r}
zero_counts <- qc_filter_zerocounts(gimap_dataset)
```
...
```{r}
histograms <- qc_plasmid_histogram(gimap_dataset)
```

```{r}
histograms$plasmid_hist_nocutoff
```

The stats used to find the cutoff are in this table

```{r}
histograms$plasmid_stats
```
...
```{r}
histograms$plasmid_hist_cutoff
```
...
```{r}
qc_combine_filters(zero_counts$filter,
                   histograms$plasmid_filter,
                   use_combined = TRUE)
```
