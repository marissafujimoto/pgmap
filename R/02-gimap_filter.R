...
#' runs both the zero count (across all samples) and the low plasmid cpm filters,
#'  but users can select a subset of these filters or even adjust the behavior of each filter
...
#' Potentially in the future also `rep_variation`, `zero_in_last_time_point` or a
#'  vector that includes multiple of these filters.
#' @param filter_zerocount_target_col default is NULL; Which sample column(s)
#' should be used to check for counts of 0? If NULL and not specified, downstream
...
#' are the final time point replicates; If NULL, the last 3 sample columns are used.
#' This is only used by this function to save a list of which pgRNA IDs have a zero
#' count for all of these samples.
...
#' construct before the construct is filtered when using a combination of filters
...
gimap_filter <- function(.data = NULL,
  ...
  if (!("gimap_dataset" %in% class(gimap_dataset))) {
    stop("This function only works with gimap_dataset objects which can be made with the setup_data() function.")
  }
  ...
  # (no matter how many there are, and ignores the NULLs) using the reduce function
  # then it finds the row sum (how many are filters flagged each construct e.g., number of TRUE in each row),
  # and finally compares the row sum to the `min_n_filters` parameter to report
  # TRUEs and FALSEs according to whether each construct is flagged by the minimum number of required filters
  ...
  #* ADD any new filter's name here* as an additional column name; START with "Filter"
  ...
  gimap_dataset$filtered_data$removed_pg_ids <- cbind(
    gimap_dataset$metadata$pg_ids[combined_filter, ],
    one_filter_df[combined_filter, ]
  ) %>%
    # add the IDs as a column together with the TRUEs and FALSEs for each filter, focusing only on pgRNAs which are in some way flagged for removal
    pivot_longer(starts_with("filter_"),
      # pivot longer so that IDs are repeated and filter names are listed in a column and the last column (`boolVals`) are TRUEs and FALSEs
      names_to = "filter_name",
      values_to = "bool_vals"
    ) %>%
    # drop rows where boolVals is false, so this leaves only filters that flagged a pgRNA for removal
  ...
  if (is.null(filter_replicates_target_col)) {
    filter_replicates_target_col <-
      c((ncol(gimap_dataset$transformed_data$log2_cpm) - 2):ncol(gimap_dataset$transformed_data$log2_cpm))
  } # last 3 columns of the data

  gimap_dataset$filtered_data$all_reps_zerocount_ids <- gimap_dataset$metadata$pg_ids[
    unlist(
      ...
      gimap_dataset$raw_counts[, filter_replicates_target_col] %>%
        ...
        # Use pivot longer to put the sample names of the samples in a `name` column
        ...
        # summarize the number of replicates that have a count of 0 for each row/pgRNA construct
        summarize(numzero = sum(counts == 0)) %>%
        # filter to include only pgRNA constructs that have a zero count for all replicates
...
#' the number and percent of pgRNA which have a count zero for at least one sample/time point
#' @examples \dontrun{
#' gimap_dataset <- get_example_data("gimap")
...
qc_filter_zerocounts <- function(gimap_dataset, filter_zerocount_target_col = NULL) {
  ...
  if (!all(filter_zerocount_target_col %in% 1:ncol(gimap_dataset$raw_counts))) {
    stop("The columns selected do not exist. `filter_zerocount_target_col`
         needs to correspond to the index of the columns in `gimap_dataset$raw_counts` that you need to filter by")
  }

  counts_filter <- data.frame(gimap_dataset$raw_counts[, filter_zerocount_target_col]) %>%
    map(~ .x %in% c(0)) %>%
    reduce(`|`)
...
#' @param filter_plasmid_target_col default is NULL, and if NULL, will select the
#' first column only; this parameter specifically should be used to specify the plasmid column(s) that will be selected
#' @importFrom magrittr %>%
...
qc_filter_plasmid <- function(gimap_dataset, cutoff = NULL, filter_plasmid_target_col = NULL) {
  ...
  if (!all(filter_plasmid_target_col %in% 1:ncol(gimap_dataset$transformed_data$log2_cpm))) {
    stop("The columns selected do not exist. `filter_plasmid_target_col` needs
         to correspond to the index of the columns in `gimap_dataset$transformed_data$log2_cpm` that you need to filter by")
  }

  plasmid_data <- data.frame(gimap_dataset$transformed_data$log2_cpm[, filter_plasmid_target_col]) %>%
    `colnames<-`(rep(c("plasmid_log2_cpm"), length(filter_plasmid_target_col))) %>%
    clean_names()

  if (length(filter_plasmid_target_col > 1)) {
    # if more than one column was selected, collapse all of the columns into the same vector using pivot_longer to store in a df with the name of the rep and number for row/construct
  ...
  if (length(filter_plasmid_target_col > 1)) {
    # if more than one column was selected, take collapsed/pooled data and compare it to the cutoff
