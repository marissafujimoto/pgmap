...
#' @param dir a file path to a directory where the bam files associated with the sample names will be stored.
...
grab_paired_files <- function(dir, sample_names, time = TRUE) {
  ...
  # Make a data frame
  sample_names_df <- data.frame(sample_name = sample_names, dir)

  # Get the file paths for each sample name
  sample_files <- furrr::future_pmap(sample_names_df, function(sample_name, dir) {
    files <- list.files(path = dir, pattern = sample_name, full.names = TRUE)
